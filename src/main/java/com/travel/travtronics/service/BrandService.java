...
import com.travel.travtronics.eserv.repository.BrandRepository;
...
public class BrandService {
	...
	public APIResponse createBrand(Brand model) {
		try {
			List<Brand> list = new ArrayList<>();
			Brand save = brandrepository.save(model);
			list.add(save);
	...
	public APIResponse listBrand(Long organizationId) {
		try {
			List<Brand> list = brandrepository.findAllByOrganizationId(organizationId);
	...
	public APIResponse editBrand(Brand model) {
		try {
			Optional<Brand> opt = brandrepository.findById(model.getId());
			if (opt.isPresent()) {
	...
	public APIResponse getBycustomerId(Long customerId) {
		try {
			List<Brand> list = brandrepository.findByCustomerId(customerId);
			if (!list.isEmpty()) {
				return new APIResponse(HttpStatus.OK.value(), HttpStatus.OK.name(), list);
			} else {
				return new APIResponse(HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.name(), new ArrayList<>());
	...
	public APIResponsePaging getPaginationByOrganization(Long organizationId, int pageNo, int pageSize, String sortBy,
		...
		if (organizationId != null) {
			Page<Brand> findByOrganizationId = brandrepository.findByOrganizationId(organizationId, pageable);
